...
Exchangeability is not likely to hold true because there are all kinds of factors that determine whether a person is going to become a smoker or not, like family history, region, mental health, etc,
The model does limit some of the these issues with assigning smoking and non-smoking as random, but cannot fully disregard all the confounders and inherent bias because people who smoke during pregnancy have similarities that people that re non-smokers do not share and visa versa.

To sum up, the matching model tries to diminish bias by matching people with similar features, but cannot eliminate that the assumption of exchangeability does not fully hold up.
...
Like stated above, because the background variables were not stratified into the randomisation process, I would still want to analyse adjusting for the background features as well, but because it was randomly assigned, I could adjust for the effects those backround charecteristics have on birthweight knowing that the background feature did not affect whether they were smoking during pregnancy or not.  

I could either stratify out the different background charecteristics and see between race, education, and age or weigh the outcome based on these charecteristics, which is doable because group selection was random and it is therefore more direct effects.

I would personally like to see the results of the random trial from the raw data, then weighted based on differences for each group.  
Then I would like to see stratified results comparing race, education, etc. to see impact on differect groups.  
***
